...
template <class T> constexpr std::int64_t storage_max() {
...
template <class T> std::int64_t block_metric(span2d::Plane<const T> a, span2d::Plane<const T> b, BlockMetric op) {
  validate_plane(a);
  validate_plane(b);
...
namespace detail {
template <class T> void half_horizontal(span2d::Plane<const T> src, span2d::Plane<T> dst, int sharp, std::int64_t max) {
  for (int y = 0; y < src.height(); ++y) {
...
template <class T> void half_vertical(span2d::Plane<const T> src, span2d::Plane<T> dst, int sharp, std::int64_t max) {
  for (int y = 0; y < src.height(); ++y) {
    if (y == src.height() - 1) {
...
BlockError block_error(const SamplingGeometry &g, BlockRegion b, const SamplingFrames<T> &frames, MotionVector vector,
  ...
  for (int k = 0; k < count; ++k) {
    ...
    errors[k] = neo_mv::simd::block_metric(source, reference, k == 0 ? metric : BlockMetric::sad);
  }
...
namespace neo_mv {
template <class T> struct HighwayKernels {
