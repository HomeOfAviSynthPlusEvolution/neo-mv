...
struct DctDistanceParts {
  std::int64_t coefficient_l1, dc_abs;
};

// Float32 DCT-II distance for all supported integer input formats:
// quantize each image independently, restore
...
class DctWorkspace {
  ...
  int stride_;
  std::int64_t sum_ = 0;
  std::vector<float> input_, rows_, transformed_;
    ...
    for (int y = 0; y < height_; ++y)
      for (int x = 0; x < width_; ++x) {
        ...
        if (value < 0 || value > maximum_)
          throw std::invalid_argument("DCT sample outside integer pixel range");
        sum_ += std::uint16_t(value);
        input_[std::size_t(y) * stride_ + x] = float(value) - float(midpoint_);
      }
  }
...
public:
  ...
  std::int64_t compare(span2d::Plane<const T> plane) {
    return scale_distance(compare_parts(plane), 4);
  }
};
