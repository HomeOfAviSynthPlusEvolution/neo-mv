...
5. abs(ex_i)>G, using the updated map and the horizontal residual expression in [global fitting](kernel-global-fit.md).
6. abs(ey_i)>G, using the updated map and the vertical residual expression in global fitting. Compute this residual only if the horizontal test did not reject the block.
...
- If the first four tests pass and the updated horizontal residual is 3 with G=2, the result is weight zero. Do not evaluate the vertical residual for that block, including when that otherwise-unused expression would overflow. If the horizontal residual passes, a non-finite required vertical residual is a frame error.
