...
| `delta` | 整数 | `1` | 正的距离步进；各成员偏移为 +delta、−delta、+2delta、−2delta……。 |
...
两个宿主均返回剪辑数组，顺序为 `[+D, -D, +2D, -2D, …, +RD, -RD]`。每个成员的输出行为同 [Analyse](analyse.md)。索引从 0 开始，不是交错存放的单个剪辑。
...
公式、舍入及数值示例见 [AnalyseMany 计算原理](../../knowledge/zh-CN/analyse-many.md)。
