...
`chroma=true` 时，色度在所有模式下都使用 SAD；`metric` 只改变亮度度量。DCT 对全部受支持尺寸和整数位深统一使用 float32 运算，包括 8 位 8×8。AC 对算出的 float32 值做最近偶数舍入，DC 从精确整数样本和截断得到。允许与 double 或历史整数 DCT 实现存在微小数值差异，代价接近的候选可能因此选出不同向量。它不是未量化 DCT 系数的直接距离。
...
以下五种新模式仅接受 8–16 位整数。所有 SATD 家族模式都要求块宽、高能被 4 整除，即使权重为零也不放宽尺寸限制；DCT 家族支持全部合法块尺寸。基础距离沿用上面的定义。
