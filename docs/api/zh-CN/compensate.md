...
VapourSynth：`core.neo_mv.Compensate`；AviSynth：`neo_mv_Compensate`。参数顺序：
...
| `thsad` | 整数 | `10000` | 非负 int64 块阈值；误差严格小于缩放阈值才使用参考块。 |
...
返回单个剪辑，尺寸、格式、帧数、帧率与 `clip` 相同，保留源帧属性。AviSynth 从第一个输入传递音频和场序。 参考不可用或被场景判断拒绝时整帧复制 clip；可用块按 thsad 选择当前或参考样本，然后合成重叠区。
...
fields=true 要求 pel>1 和所需场序。所有可能的矢量、时间缩放及场偏移须有有效采样支持；实际非法采样或完整数组损坏会报错，不作为回退。允许 delta=0。

## 计算原理

公式、舍入及数值示例见 [Compensate 计算原理](../../knowledge/zh-CN/compensate.md)。
