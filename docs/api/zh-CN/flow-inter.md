...
| `time` | 浮点数 | `50.0` | 第 n 帧与 n+d 帧之间的位置百分比，有限值 0～100。 |
...
返回单个剪辑，尺寸、格式、帧数、帧率与 `clip` 相同，保留源帧属性。AviSynth 从第一个输入传递音频和场序。 运动不可用时，blend=true 混合钳位后的原始端点图像，否则复制左端图像。
...
time 须为 0～100，ml 须为正。一个主场不可用也会校验另一个。额外观测缺失时使用普通插值；损坏观测或非法采样会报错，不会用混合掩盖。

## 计算原理

公式、舍入及数值示例见 [FlowInter 计算原理](../../knowledge/zh-CN/flow-inter.md)。
