...
VapourSynth：`core.neo_mv.FlowBlur`；AviSynth：`neo_mv_FlowBlur`。参数顺序：
...
| `blur` | 浮点数 | `50.0` | 控制轨迹长度的百分比，转为 float32 后须为有限值 0～200。 |
| `prec` | 整数 | `1` | 正的采样步进参数；增大将减少轨迹采样点，可能只剩中心。 |
...
返回单个剪辑，尺寸、格式、帧数、帧率与 `clip` 相同，保留源帧属性。AviSynth 从第一个输入传递音频和场序。 需要两侧时间方向；边界或运动不可用时复制原帧。
...
blur 须为有限值 0～200，prec 至少为 1。blur=0 仍校验所需矢量场和 Super。每个轨迹点都须有有效相位支持，不会缩短非法轨迹来隐藏错误。

## 计算原理

公式、舍入及数值示例见 [FlowBlur 计算原理](../../knowledge/zh-CN/flow-blur.md)。
