...
Visualize block-vector magnitude.

## Calling the function

VapourSynth: `core.neo_mv.VectorLengthMask`. AviSynth: `neo_mv_VectorLengthMask`. Parameter order:

```text
VectorLengthMask(vectors [, ml, gamma, time, scval, thscd1, thscd2, prefix])
```
...
result = core.neo_mv.VectorLengthMask(v[0])
result.set_output()
```
...
result = neo_mv_VectorLengthMask(v[0])
return result
```
...
See [VectorLengthMask: computation](../../knowledge/en/vector-length-mask.md) for formulas, rounding and numerical examples.
