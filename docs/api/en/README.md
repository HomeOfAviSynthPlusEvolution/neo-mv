...
The `metric` parameter of Analyse, AnalyseMany, and Recalculate selects `"sad"`, `"satd"`, integer-only `"dct"`, or integer-only local/global mixed modes. See [Analyse](analyse.md#matching-metric) for format and block-size restrictions. Functions consuming analysis data use its stored block errors, including SATD/DCT and mixed errors, without converting them to pixel SAD.
