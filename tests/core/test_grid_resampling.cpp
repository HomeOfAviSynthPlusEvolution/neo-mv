...
void examples() {
  ...
  GridResamplingPlan({2, 1, 4, 4, 0, 0, 8, 4}).resize(input.read(), output.view(), 8);
  GridResamplingPlan({2, 1, 4, 4, 0, 0, 6, 4}).resize(input.read(), cropped.view(), 8);
  ...
  GridResamplingPlan({2, 1, 4, 4, 0, 0, 8, 4}).resize(signed_input.read(), signed_output.view(), 16);
  for (int x = 0; x < 8; ++x)
  ...
  GridResamplingPlan({2, 2, 3, 3, 0, 0, 6, 6}).resize(square.read(), enlarged.view(), 8);
  CHECK(enlarged.view().row(2)[2] == 1); // Double horizontal rounding would give zero.
  Buffer<float> fp(2, 1), out(8, 4);
  fp.view().row(0)[0] = 0;
  fp.view().row(0)[1] = 1;
  GridResamplingPlan({2, 1, 4, 4, 0, 0, 8, 4}).resize(fp.read(), out.view(), 32);
  ...
  for (float value : {std::numeric_limits<float>::max(), -std::numeric_limits<float>::max(),
    ...
    GridResamplingPlan({2, 1, 3, 3, 0, 0, 8 - 2, 4 - 1}).resize(fp.read(), out.view().subplane(0, 0, 6, 3), 32);
    for (int x = 0; x < 6; ++x)
...
void exact_wide_arithmetic() {
  ...
  GridResamplingPlan({1, 1, INT32_MAX, INT32_MAX, 0, 0, 1, 1}).resize(input.read(), output.view(), 16);
  ...
  GridResamplingPlan({INT32_MAX, INT32_MAX, INT32_MAX, INT32_MAX, 0, 0, 1, 1});
}

void failures() {
  rejects([] { GridResamplingPlan({2, 1, 4, 4, 0, 0, 9, 4}); });
  rejects([] { GridResamplingPlan({2, 1, 4, 4, 4, 0, 8, 4}); });
  Buffer<float> input(2, 1), output(8, 4);
  const GridResamplingPlan plan({2, 1, 4, 4, 0, 0, 8, 4});
  input.view().row(0)[0] = 0;
